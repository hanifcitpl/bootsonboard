...
//		echo "deleted itme  --->".$itemId;
//		echo "<br>";
		
        if ($item) {
			
			// delete tax product from cart hello moon
				...
				$collection = Mage::getModel('guest/guesttemp')->getCollection()->addFieldToFilter('quote_id',$quoteId)->addFieldToFilter('productid',$Id)->getData();
				 ...
				 if(count($quoteItems)>0){
					 ...
					 foreach($quoteItems as $oneItem){
						 	...
						 	if($oneItem->getProductId()==$product_id){
									...
									$taxes[$oneItem->getId()] = $oneItem->getQty();
							}
					 }
...
//					print_r($taxes);
					...
					$attribute_value = $product->getResource()->getAttribute('paying_for')->getFrontend()->getValue($product);
					
					if(count($taxes) > 1){
						
							$search = array_search($attribute_value,$taxes);
							 ...
							 if(!empty($search)){
										...
										$collection 	= Mage::getModel('guest/guesttemp')->getCollection()->addFieldToFilter('quote_id',$quote_id)->addFieldToFilter('productid',$Id)->getData();
																		 
										$duobleproduct = $this->getItemById($search);
									
										$duobleproduct->setQuote($this);

										$this->setIsMultiShipping(false);
										$duobleproduct->isDeleted(true);
										if ($duobleproduct->getHasChildren()) {
											
											foreach ($duobleproduct->getChildren() as $child) {
												$child->isDeleted(true);
											}
										}
							
										$parent = $duobleproduct->getParentItem();
										...
										$sql = "Delete from guest_temp where quote_id = '".$quoteId."' and productid='".$Id."'";
										$db_write->query($sql); 
										
										//echo "tax product deleted successfully";
							  }else{
								  
										foreach($quoteItems as $oneItem){
																							 
	//													echo $oneItem->getId()."------".$oneItem->getProductId();
	//													echo "<br>";
														
														if($oneItem->getProductId()==$product_id){
															 
																	//echo $item->getProduct()->getTypeId()."------>";
																	$session = Mage::getSingleton('checkout/session');
																	$quote_id       = $session->getQuoteId();
																	$collection 	= Mage::getModel('guest/guesttemp')->getCollection()->addFieldToFilter('quote_id',$quote_id)->addFieldToFilter('productid',$Id)->getData();
	...
	//																	echo $oneItem->getQty()."------".$attribute_value;
	//																	echo "<br>";
								
																		$qty = $oneItem->getQty() - $attribute_value;
																		...
																		$quotsecond = Mage::getSingleton('checkout/session')->getQuote();
																		
																		if($qty > 0){
								
																				
																				$quotsecond->updateItem($oneItem->getId(), array( 'qty' => $qty));
																				$quotsecond->save();
																				
																		}else{
																			
																				$rmproduct = $this->getItemById($oneItem->getId());
																			
																				$rmproduct->setQuote($this);
								
																				$this->setIsMultiShipping(false);
																				$rmproduct->isDeleted(true);
																				if ($rmproduct->getHasChildren()) {
																					foreach ($rmproduct->getChildren() as $child) {
																						$child->isDeleted(true);
																					}
																				}
																	
																				$parent = $rmproduct->getParentItem();
...
//						print_r($oneItem);
						...
						if($oneItem->getProductId()==$product_id){
							...
							//if($Id && ($item->getProduct()->getTypeId() == "configurable" || $item->getProduct()->getTypeId() == "simple")){
